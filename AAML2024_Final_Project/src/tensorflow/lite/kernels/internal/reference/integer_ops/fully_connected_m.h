...
inline void FullyConnected(
  ...
  for (int b = 0; b < batches; ++b) {
    ...
    for (int out_c = 0; out_c < output_depth; ++out_c) {
      int32_t acc = cfu_op0(0, 0, 0); // resets acc
      for (int d = 0; d < accum_depth; d += 4) {
        ...
        acc = cfu_op0(1, input_val, (filter_val + filter_offset));
      }

      acc = cfu_op0(2, bias_data[out_c], output_offset);
      acc = cfu_op0(3, output_multiplier, output_shift);
