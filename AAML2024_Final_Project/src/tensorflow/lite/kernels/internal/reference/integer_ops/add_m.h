...
inline void ElementWise_m(
  ...
  [[maybe_unused]]uint32_t cfu_result = cfu_op0(0, input_offset, params.output_shift);
  __asm volatile("NOP");
  cfu_result = cfu_op0(1, params.input1_multiplier, params.output_multiplier);
...
inline uint32_t AddFunc_m(uint32_t x, uint32_t y, const ArithmeticParams& params) {
  uint32_t cfu_result = cfu_op0(2, x, y);
  __asm volatile("NOP");
  return cfu_result;
}
